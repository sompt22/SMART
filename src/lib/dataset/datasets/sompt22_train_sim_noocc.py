...
class SOMPT22_TRAIN_SIM_NOOCC(GenericDataset):
  num_classes = 1
  num_categories = 1
  ...
  def __init__(self, opt, split):
    ...
    print('Using SOMPT22_TRAIN_SIM_NOOCC {}'.format(split))

    data_dir = os.path.join(opt.data_dir, 'sompt22-train-sim-noocc')
    ...
    super(SOMPT22_TRAIN_SIM_NOOCC, self).__init__(opt, split, ann_path, img_dir)

    self.num_samples = len(self.images)
    print('Loaded SOMPT22_TRAIN_SIM_NOOCC {} {} samples'.format(split, self.num_samples))
  ...
  def save_results(self, results, save_dir):
    results_dir = os.path.join(save_dir, 'results_sompt22_train_sim_noocc')
