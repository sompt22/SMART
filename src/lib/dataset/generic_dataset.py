...
class GenericDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    pre_cts, track_ids, embedding_vectors = None, None, None
    if opt.tracking:
      ...
      pre_hm, pre_cts, track_ids, embedding_vectors = self._get_pre_dets(
    ...
    for k in range(num_objs):
      ...
      self._add_instance(
        ...
        calib, pre_cts, track_ids, embedding_vectors)
  ...
  def _get_pre_dets(self, anns, trans_input, trans_output):
    ...
    for ann in anns:
      ...
      if (h > 0 and w > 0):
        ...
        if self.opt.know_dist_weight:
          embedding_vectors.append(ann['embedding'] if 'embedding' in ann else np.zeros(self.opt.embedding_dim, dtype=np.float32))
        if reutrn_hm:
    ...
    return pre_hm, pre_cts, track_ids, embedding_vectors
  ...
  def _init_ret(self, ret, gt_det):
    ...
    if 'embedding' in self.opt.heads:
      ...
      gt_det['ids'] = []  
      if self.opt.know_dist_weight:
        ret['vectors'] = np.zeros((max_objs, self.opt.embedding_dim), dtype=np.float32)
        ret['vectors_mask'] = np.zeros((max_objs, ), dtype=np.float32)
        gt_det['vectors'] = [] 
  ...
  def _add_instance(
    ...
    aug_s, calib, pre_cts=None, track_ids=None, embedding_vectors=None):
    h, w = bbox[3] - bbox[1], bbox[2] - bbox[0]
    if h <= 0 or w <= 0:
      return
    radius = gaussian_radius((math.ceil(h), math.ceil(w)))
    ...
    if 'embedding' in self.opt.heads:
        ...
        if self.opt.know_dist_weight:
            if ann['embedding'] in embedding_vectors:
                ...
                gt_det['vectors'].append(np.zeros(self.opt.embedding_dim, dtype=np.float32))    
  ...
  def _format_gt_det(self, gt_det):
    if (len(gt_det['scores']) == 0):
      ...
      if self.opt.know_dist_weight:
        gt_det['vectors'] = np.zeros(self.opt.embedding_dim, dtype=np.float32)
