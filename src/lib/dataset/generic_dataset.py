...
class GenericDataset(data.Dataset):
  ...
  def __init__(self, opt=None, split=None, ann_path=None, img_dir=None):
    ...
    if ann_path is not None and img_dir is not None:
      ...
      self.track_id_base = self._resolve_track_id_base()
  ...
  def _resolve_track_id_base(self):
    if not hasattr(self, 'coco') or not hasattr(self.coco, 'dataset'):
      return 0
    dataset = self.coco.dataset
    if 'track_id_base' in dataset:
      try:
        return int(dataset['track_id_base'])
    ...
    track_ids = [
      int(ann['track_id']) for ann in annotations
      if 'track_id' in ann and isinstance(ann['track_id'], (int, float))
      and int(ann['track_id']) >= 0
    ]
    if not track_ids:
      return 0

    min_tid = min(track_ids)
    max_tid = max(track_ids)
    total_id = dataset.get('total_id', None)
    ...
    # Heuristic fallback for legacy annotations generated by this repo's
    # converters: IDs are positive and total_id equals the maximum global ID.
    if min_tid >= 1:
      if total_id is None or max_tid <= total_id:
        return 1
    return 0

  def _track_id_to_tid(self, track_id):
    if track_id is None:
      return None
    try:
      ...
      return None
    tid = track_id - self.track_id_base
    if tid < 0:
      return None
    return tid
  ...
  def _add_instance(
    ...
    if 'embedding' in self.opt.heads:
      if ann_track_id in track_ids:
        tid = self._track_id_to_tid(ann_track_id)
        if tid is not None:
          ret['tid_mask'][k] = 1
          ret['tid'][k] = tid
  ...
  def fake_video_data(self):
    ...
    self.coco.dataset['track_id_base'] = 1
