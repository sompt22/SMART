...
def extract_data(line, current_epoch):
    ...
    loss_pattern = r'tot ([\d.]+) \| hm ([\d.]+) \| wh ([\d.]+) \| reg ([\d.]+) \| embedding ([\d.]+) \| ltrb_amodal ([\d.]+) \| tracking ([\d.]+) \| time ([\d.]+)'
    ...
    if loss_match:
        loss_data = {
            ...
            'embedding': float(loss_match.group(5)),
            'ltrb_amodal': float(loss_match.group(6)),
            'tracking': float(loss_match.group(7)),
            'time': float(loss_match.group(8)),
        }
...
def plot_losses(training_losses, validation_losses, save_path):
    ...
    for i, metric in enumerate(['tot', 'hm', 'wh', 'reg', 'embedding', 'ltrb_amodal', 'tracking', 'time']):
        ax = axes[i]
